...
public class Vimeo extends Site {
    ...
    public Vimeo(String query, int targetTime) {
    ...
    public void run() {
        ...
        try {
            ...
            Arrays.asList(requestedPage.select("script").get(15).toString().split("\n")).forEach(e -> {
                if (e.startsWith("    vimeo.config")) processedLine.set(e.trim().substring(45));
            });
            ...
            Debug.log("Failed to connect to: " + Resources.vimeoSearch + query);
        }
